"""RAG pipeline using Haystack, Pinecone, and Azure Files.

Ingests documents from an Azure file share, converts files to Haystack
Documents, indexes into Pinecone via a Haystack indexing pipeline, and
...
def embed_and_index(chunks: list[Document]) -> PineconeDocumentStore:
    ...
    Returns: A PineconeDocumentStore connected to the populated index.
    """
...
def build_query_pipeline(
    ...
    relevant documents from Pinecone, and generates an answer via Azure OpenAI.

    Args:
        document_store: PineconeDocumentStore to retrieve from.
